...
void do_load(int n)
{
    /* need to check if n is within a valid range or registers. */
    int number = vm.reg[n];
    do_push(number);
...
void do_jmp(int n)
{
    if(n > MAX_INSTS)
    {
        fprintf(stdout, "Too Many Instructions\n");
    }
    else
    {
        vm.ip = n;
...
void do_add(void)
{
    ...
    number1 = vm.stack[vm.sp - 1];
    do_pop();
    number2 = vm.stack[vm.sp - 1];
    do_pop();
    sum = number1 + number2;
    do_push(sum);
}
